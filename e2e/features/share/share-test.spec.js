...
test('Share link clipboard with existing time query string param in the page url will have the same serialized time', async () => {
  ...
  const expectedDate = '2018-12-31'
  const queryString = 'http://localhost:3000/?t=' + expectedDate
  await page.goto(queryString)
  await shareToolbarButton.click()
  const url = await page.url()
  expect(url).toContain('t=')
  await expect(shareLinkInput).toHaveValue('http://localhost:3000/?t=' + expectedDate + '-T00%3A00%3A00Z')
})
